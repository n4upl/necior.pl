...
//$scriptUrl = "http://".$_SERVER["serwer2107818.home.pl"].$_SERVER["form-handler.php"];
$scriptUrl = "http://necior.pl/form-handler.php";
// Email "subject"
...
if (!empty($_POST)){
	...
	foreach($_POST as $fieldKey=>$fieldValue){
		if(!empty($fieldValue)){
			switch($fieldKey){
				case "name":
					$message .= "<b>Imię:</b> ".$fieldValue.'<br />';
				...
				case "coupon":
					...
					continue 2;
				case "message":
					$message .= "<b>Wiadomość:</b> ".str_replace("\n", "<br />", $fieldValue).'<br />';
					$hubSpotContact["message"] = $fieldValue;
					continue 2;
				case "send_copy":
					$message .= "<b>Użytkownik zaznaczył:</b> Aby wysłać mu kopię<br />";
					continue 2;
				case "remember":
